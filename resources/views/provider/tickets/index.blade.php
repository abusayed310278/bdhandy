...
<div class="space-y-5 text-sm">
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    ...
    <a href="{{ route('provider.tickets.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white text-sm font-bold hover:bg-primary-600 transition">
  ...
  @if($tickets->isEmpty())
    <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center">
      ...
      <a href="{{ route('provider.tickets.create') }}" class="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary-500 text-white text-sm font-semibold hover:bg-primary-600 transition">Open Ticket</a>
    </div>
  @else
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <tr class="hover:bg-slate-50/50 transition">
              ...
              <td class="px-5 py-4 text-center">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-{{ $pc }}-50 text-{{ $pc }}-700">{{ $ticket->priority }}</span>
              </td>
              <td class="px-5 py-4 text-center">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-{{ $sc }}-50 text-{{ $sc }}-700">{{ $ticket->status }}</span>
              </td>
              <td class="px-5 py-4 text-right">
                <a href="{{ route('provider.tickets.show', $ticket) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary-50 text-primary-700 text-xs font-medium hover:bg-primary-100 transition">
