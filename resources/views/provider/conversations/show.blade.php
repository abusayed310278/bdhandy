...
<div class="max-w-3xl space-y-4 text-sm">
  ...
  <div class="flex items-center gap-3">
    <a href="{{ route('provider.conversations.index') }}" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition">
    ...
    <div>
      <p class="font-semibold text-slate-900">{{ $customer?->name ?? 'Customer' }}</p>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl border border-slate-200 flex flex-col" style="height:calc(100vh - 260px); min-height:400px">
    ...
    <div class="border-t border-slate-100 p-4" x-data="chat({{ $conversation->id }}, {{ auth()->id() }})">
      <div class="flex items-end gap-3">
        <form id="msg_form" action="{{ route('provider.conversations.message', $conversation) }}" method="POST" class="flex-1 flex items-end gap-3" @submit.prevent="sendMessage">
          @csrf
