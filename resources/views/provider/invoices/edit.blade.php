...
<div class="max-w-2xl space-y-6 text-sm"
     x-data="{
       ...
       adjustment:     {{ (float)($invoice->adjustment_amount ?? 0) }},
  ...
  <div>
    <a href="{{ route('provider.invoices.show', $invoice) }}" class="text-xs text-slate-500 hover:text-primary-600 flex items-center gap-1 mb-1.5">
      <svg class="w-3.5 h-3.5 rtl-flip" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>
      Back to Invoice
    </a>
    <h2 class="text-xl font-bold text-slate-900">Edit Invoice</h2>
    <p class="text-xs text-slate-400 mt-0.5">{{ $invoice->invoice_number }} · {{ $invoice->serviceRequest?->request_number }}</p>
  </div>
  ...
  <form action="{{ route('provider.invoices.update', $invoice) }}" method="POST" class="space-y-5">
    @csrf @method('PUT')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
        ...
        <div>
          ...
          <select name="currency_id" required class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
            @foreach($currencies as $cur)
              <option value="{{ $cur->id }}" {{ $invoice->currency_id == $cur->id ? 'selected' : '' }}>{{ $cur->symbol }} {{ $cur->name }}</option>
            @endforeach
          </select>
        </div>
        <div>
          ...
          @foreach(['none' => 'No Discount', 'fixed' => 'Fixed Amount', 'percent' => 'Percentage'] as $val => $label)
            <label class="flex-1 cursor-pointer">
              ...
              <div class="text-center py-2 px-2 rounded-xl border-2 text-xs font-semibold transition"
                   :class="discountType === '{{ $val }}' ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'">{{ $label }}</div>
            </label>
          @endforeach
        </div>
        <div x-show="discountType !== 'none'" x-transition x-cloak>
          <div class="relative">
            <input type="number" name="discount_value" step="0.01" min="0"
                   x-model.number="discountValue" @input="recalc()" placeholder="0"
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Tax Label</label>
          <input type="text" name="tax_label" value="{{ old('tax_label', $invoice->tax_label) }}" placeholder="e.g. VAT, GST"
                 class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
        <div>
          ...
          <div class="relative">
            <input type="number" name="tax_rate" step="0.01" min="0" max="100"
                   x-model.number="taxRate" @input="recalc()" placeholder="0"
          ...
          <p class="mt-1.5 text-xs text-slate-400" x-show="taxAmount > 0">Tax: <span class="font-bold text-slate-600" x-text="taxAmount.toFixed(2)"></span></p>
        </div>
      </div>

      <div class="grid sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Adjustment (+ or −)</label>
          <input type="number" name="adjustment_amount" step="0.01"
                 x-model.number="adjustment" @input="recalc()" placeholder="0"
                 class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Adjustment Note</label>
          <input type="text" name="adjustment_note" value="{{ old('adjustment_note', $invoice->adjustment_note) }}" placeholder="Reason for adjustment"
                 class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
        ...
        <div>
          ...
          <select name="payment_status" required class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
            @foreach(['draft' => 'Draft', 'pending' => 'Pending', 'due' => 'Due', 'partial' => 'Partial', 'paid' => 'Paid'] as $val => $label)
              <option value="{{ $val }}" {{ $invoice->payment_status === $val ? 'selected' : '' }}>{{ $label }}</option>
            @endforeach
          </select>
        </div>
        <div>
          ...
          <select name="payment_method" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
            ...
            @foreach(['cash' => 'Cash', 'card' => 'Card', 'online' => 'Online / Mobile Banking', 'cheque' => 'Cheque', 'bank_transfer' => 'Bank Transfer', 'other' => 'Other'] as $val => $label)
              <option value="{{ $val }}" {{ $invoice->payment_method === $val ? 'selected' : '' }}>{{ $label }}</option>
            @endforeach
          </select>
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Reference / Card No / Cheque No</label>
          <input type="text" name="payment_reference" value="{{ old('payment_reference', $invoice->payment_reference) }}"
      ...
      <div>
        ...
        <textarea name="notes" rows="3" placeholder="Additional notes for the customer…"
                  class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 resize-none">{{ old('notes', $invoice->notes) }}</textarea>
      </div>
    </div>
