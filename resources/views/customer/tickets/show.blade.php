...
<div class="max-w-3xl space-y-5 text-sm">
  ...
  <div class="flex items-start justify-between gap-3">
    ...
    <div class="flex items-center gap-2 shrink-0">
      <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-{{ $pc }}-50 text-{{ $pc }}-700">{{ $ticket->priority }}</span>
      <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-{{ $sc }}-50 text-{{ $sc }}-700">{{ $ticket->status }}</span>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl border border-slate-200 p-5">
    ...
    <form action="{{ route('customer.tickets.reply', $ticket) }}" method="POST" class="space-y-3">
      @csrf
  ...
  @else
    <div class="bg-slate-50 rounded-2xl border border-slate-200 p-4 text-center">
      <p class="text-xs text-slate-500">This ticket is {{ $ticket->status }}. <a href="{{ route('customer.tickets.create') }}" class="text-primary-600 hover:underline">Open a new ticket</a> if you need further help.</p>
    </div>
  @endif
</div>
@endsection
