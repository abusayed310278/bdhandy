...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h3 class="text-xl font-bold text-slate-900">Service Categories</h3>
            <p class="text-sm text-slate-500 mt-1">Manage and organize your platform's service categories</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white text-sm font-bold hover:bg-primary-600 transition shadow-soft">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.categories.index') }}" method="GET" class="flex items-center gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition outline-none" placeholder="Search by name, slug or translations...">
            </div>
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 transition">
                Search
            </button>
            @if(request('search'))
                <a href="{{ route('admin.categories.index') }}" class="p-2.5 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition" title="Clear Search">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Icon & Name</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Cover Image</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Order</th>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($categories as $category)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-6 py-4 text-slate-400 font-medium">{{ $loop->iteration + ($categories->currentPage() - 1) * $categories->perPage() }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200 shrink-0">
                                        @if($category->icon)
                                            <img src="{{ asset('storage/'.$category->icon) }}" class="w-full h-full object-cover">
                                        @else
                                            <svg class="w-5 h-5 text-slate-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>
                                        @endif
                                    </div>
                                    <div>
                                        @php
                                            $defaultLangCode = \App\Models\Language::where('is_default', true)->first()->code ?? 'en';
                                        @endphp
                                        <p class="font-bold text-slate-900 leading-none">{{ $category->getTranslation('translations', $defaultLangCode) }}</p>
                                        <p class="text-xs text-slate-400 mt-1.5">{{ $category->slug }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if($category->image)
                                    <div class="w-20 h-10 rounded-lg overflow-hidden border border-slate-200 shadow-sm">
                                        <img src="{{ asset('storage/'.$category->image) }}" class="w-full h-full object-cover">
                                    </div>
                                @else
                                    <span class="text-slate-300 text-xs italic">No image</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2 py-1 rounded-lg bg-slate-100 text-slate-600 font-bold text-xs">{{ $category->sort_order }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.categories.edit', $category->id) }}" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-500 hover:text-white transition shadow-sm" title="Edit">
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                    </a>
                                    <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Delete this category forever?')">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-slate-500 font-medium">No categories found matching your search.</p>
                                    <a href="{{ route('admin.categories.index') }}" class="text-primary-500 text-sm font-bold mt-2 hover:underline">Clear all filters</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
