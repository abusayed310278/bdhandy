...
<div class="space-y-6 text-sm">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h3 class="text-xl font-bold text-slate-900">Marketing Banners</h3>
            <p class="text-sm text-slate-500 mt-1">Manage promotional banners for web and mobile apps</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.banners.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white text-sm font-bold hover:bg-primary-600 transition shadow-soft">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.banners.index') }}" method="GET" class="flex items-center gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition outline-none" placeholder="Search by banner title...">
            </div>
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 transition">
                Search
            </button>
            @if(request('search'))
                <a href="{{ route('admin.banners.index') }}" class="p-2.5 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition" title="Clear Search">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($banners as $banner)
                        <tr class="hover:bg-slate-50/50 transition group">
                            <td class="px-6 py-4 text-slate-400 font-medium">{{ $loop->iteration + ($banners->currentPage() - 1) * $banners->perPage() }}</td>
                            <td class="px-6 py-4">
                                <div class="relative w-24 h-12 rounded-lg overflow-hidden border border-slate-200 shadow-sm">
                                    <img src="{{ asset('storage/'.$banner->image) }}" class="w-full h-full object-cover transition duration-300 group-hover:scale-110" alt="{{ $banner->title }}">
                                </div>
                            </td>
                            <td class="px-6 py-4 max-w-xs">
                                <p class="font-bold text-slate-900 truncate">{{ $banner->title ?: 'Untitled Banner' }}</p>
                                <p class="text-[10px] text-slate-500 mt-0.5 truncate">{{ $banner->link ?: 'No Link' }}</p>
                                @if($banner->start_date || $banner->end_date)
                                    <p class="text-[9px] text-primary-600 font-semibold mt-1 uppercase tracking-wider">
                                        {{ $banner->start_date ? $banner->start_date->format('d M') : 'Start' }} — {{ $banner->end_date ? $banner->end_date->format('d M') : 'End' }}
                                    </p>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="inline-flex flex-col items-center">
                                    <span class="px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 text-[10px] font-bold uppercase tracking-wider mb-1">
                                        {{ $banner->position }}
                                    </span>
                                    <span class="text-[10px] text-slate-400">{{ strtoupper($banner->type) }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.banners.edit', $banner->id) }}" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-500 hover:text-white transition shadow-sm" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500 italic">No banners found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
