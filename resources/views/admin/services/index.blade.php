...
<div class="space-y-6 text-sm">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h3 class="text-xl font-bold text-slate-900">Platform Services</h3>
            <p class="text-sm text-slate-500 mt-1">Manage core services offered within categories</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.services.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white text-sm font-bold hover:bg-primary-600 transition shadow-soft">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.services.index') }}" method="GET" class="flex flex-col md:flex-row items-center gap-3">
            <div class="relative flex-1 w-full">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition outline-none" placeholder="Search service name...">
            </div>
            <select name="category_id" class="w-full md:w-48 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white transition outline-none cursor-pointer">
                <option value="">All Categories</option>
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{ request('category_id') == $category->id ? 'selected' : '' }}>
                        {{ $category->getTranslation('translations', 'en') }}
                    </option>
                @endforeach
            </select>
            <button type="submit" class="w-full md:w-auto px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 transition">
                Filter
            </button>
            @if(request('search') || request('category_id'))
                <a href="{{ route('admin.services.index') }}" class="p-2.5 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition" title="Clear All">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition group">
                                    ...
                                    <div class="w-10 h-10 rounded-lg overflow-hidden border border-slate-200 bg-slate-50">
                                        ...
                                        @else
                                            <div class="w-full h-full flex items-center justify-center text-slate-300">
                                                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                                            </div>
                                        @endif
                                    </div>
                                    <div>
                                        ...
                                        @endphp
                                        <p class="font-bold text-slate-900">{{ $service->getTranslation('translations', $defaultLangCode)['name'] ?? 'Untitled Service' }}</p>
                                        <p class="text-[10px] text-slate-400 font-medium tracking-wide">SLUG: {{ $service->slug }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-lg bg-slate-100 text-slate-700 text-[10px] font-bold uppercase tracking-wider">
                                    {{ $service->category->getTranslation('translations', $defaultLangCode) }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.services.edit', $service->id) }}" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-500 hover:text-white transition shadow-sm" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-500 italic">No services found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
