...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h3 class="text-xl font-bold text-slate-900">Divisions</h3>
            <p class="text-sm text-slate-500 mt-1">Manage administrative divisions for each country</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.divisions.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white text-sm font-bold hover:bg-primary-600 transition shadow-soft">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.divisions.index') }}" method="GET" class="flex items-center gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition outline-none" placeholder="Search by division name or country...">
            </div>
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 transition">
                Search
            </button>
            @if(request('search'))
                <a href="{{ route('admin.divisions.index') }}" class="p-2.5 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition" title="Clear Search">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Division Name</th>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($divisions as $division)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-6 py-4 text-slate-400 font-medium">{{ $loop->iteration + ($divisions->currentPage() - 1) * $divisions->perPage() }}</td>
                            <td class="px-6 py-4">
                                <p class="font-bold text-slate-900 leading-none">{{ $division->name }}</p>
                                <p class="text-xs text-slate-400 mt-1.5 uppercase tracking-tighter">ID: #{{ $division->id }}</p>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2.5 py-1 rounded-full bg-primary-50 text-primary-700 text-[11px] font-bold uppercase tracking-wider">
                                    {{ $division->country->name }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-slate-500 font-medium text-xs">{{ $division->slug }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.divisions.edit', $division->id) }}" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-500 hover:text-white transition shadow-sm" title="Edit">
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                    </a>
                                    <form action="{{ route('admin.divisions.destroy', $division->id) }}" method="POST" onsubmit="return confirm('Delete this division? All districts and areas under it will be removed.')">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-slate-500 font-medium">No divisions found.</p>
                                    <a href="{{ route('admin.divisions.index') }}" class="text-primary-500 text-sm font-bold mt-2 hover:underline">Clear search</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
