...
<div class="space-y-6 text-sm">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h3 class="text-xl font-bold text-slate-900">Discount Coupons</h3>
            <p class="text-sm text-slate-500 mt-1">Manage promotional codes and special offers</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.coupons.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white text-sm font-bold hover:bg-primary-600 transition shadow-soft">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.coupons.index') }}" method="GET" class="flex items-center gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition outline-none" placeholder="Search by code or title...">
            </div>
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 transition">
                Search
            </button>
            @if(request('search'))
                <a href="{{ route('admin.coupons.index') }}" class="p-2.5 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition" title="Clear Search">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($coupons as $coupon)
                        <tr class="hover:bg-slate-50/50 transition group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-primary-500 group-hover:text-white transition-colors duration-300">
                                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
                                    </div>
                                    <div>
                                        <p class="font-black text-slate-900 tracking-wider uppercase">{{ $coupon->code }}</p>
                                        <p class="text-[10px] text-slate-500 font-medium">{{ $coupon->title }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="inline-flex flex-col">
                                    <span class="text-sm font-bold text-slate-900">
                                        {{ $coupon->discount_type == 'percentage' ? (int)$coupon->discount_value . '%' : number_format($coupon->discount_value, 2) }}
                                    </span>
                                    <span class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{{ $coupon->discount_type }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex flex-col items-center">
                                    <span class="text-sm font-bold text-slate-900">{{ $coupon->used_count }}</span>
                                    <span class="text-[9px] text-slate-400 font-bold uppercase">/ {{ $coupon->usage_limit ?: '∞' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex flex-col items-center">
                                    <span class="text-[10px] text-slate-700 font-bold">{{ $coupon->start_date ? $coupon->start_date->format('d M') : 'Now' }}</span>
                                    <span class="text-[9px] text-slate-400 font-bold uppercase">TO</span>
                                    <span class="text-[10px] text-slate-700 font-bold">{{ $coupon->end_date ? $coupon->end_date->format('d M') : '∞' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center">
                                    ...
                                    @elseif($coupon->status == 'expired')
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-50 text-red-700 text-[10px] font-bold uppercase tracking-wider">
                                            Expired
                                        </span>
                                    @else
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.coupons.edit', $coupon->id) }}" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-500 hover:text-white transition shadow-sm" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500 italic">No coupons found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
