...
<div x-data="{
  ...
  {{-- ════════════════════════════════════════════════════════ COVER ════════════════════════════════════════════════════════ --}}
  {{--
    ARCHITECTURE: Single relative wrapper.
    - The cover image sits at the TOP, absolutely positioned.
    - A gradient fades it out starting at exactly the 50% mark.
    - All content (logo, name, grid) flows normally BELOW the fade line.
    - The cover wrapper uses padding-top to reserve space for the top half of the image.
    This mimics the professional hero-overlap pattern used by LinkedIn, Airbnb etc.
  --}}
  <div class="relative w-full">

    {{-- Cover image: absolute, padded, top-anchored.
         aspect-[1600/890] is on the INNER div so the ratio is based
         on the actual narrowed width (viewport minus padding). --}}
    <div class="absolute inset-x-0 top-0 z-10 px-6 sm:px-14 lg:px-24 pt-4 sm:pt-6">
      {{-- Outer positioning wrapper - no overflow-hidden so share dropdown isn't clipped --}}
      <div class="w-full aspect-[1600/890] rounded-2xl sm:rounded-3xl relative" x-data="{ shareOpen: false, copied: false }">

        {{-- Image + gradient overlays — clipped separately inside --}}
        <div class="absolute inset-0 rounded-2xl sm:rounded-3xl overflow-hidden pointer-events-none">
          @if($profile->cover_photo)
            <img src="{{ asset('storage/'.$profile->cover_photo) }}" class="w-full h-full object-cover object-top select-none">
          @else
          ...
          @endif
          {{-- Top vignette for buttons readability --}}
          <div class="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-black/50 to-transparent"></div>
          {{-- Complete white fade from the bottom half --}}
          <div class="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-white via-white/90 to-transparent"></div>
        </div>


        {{-- Buttons: inside cover, top-right, z-30 so dropdown shows above --}}
        <div class="absolute top-4 right-4 z-30 flex items-center gap-2">
          ...
          <div class="relative">
            ...
            <div x-show="shareOpen"
                 ...
                 class="absolute right-0 mt-2 w-48 rounded-2xl bg-white/95 backdrop-blur-lg border border-slate-200/80 p-1.5 shadow-xl z-30 space-y-0.5"
                 style="display: none;">
              <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank" rel="noopener"
                 class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-50 transition">
                <svg class="w-4 h-4 text-blue-600 fill-current" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>
                Facebook
              </a>
              <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($profile->business_name) }}" target="_blank" rel="noopener"
                 class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-50 transition">
                <svg class="w-4 h-4 text-slate-900 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                Twitter / X
              </a>
              <a href="https://api.whatsapp.com/send?text={{ urlencode($profile->business_name . ' - ' . url()->current()) }}" target="_blank" rel="noopener"
                 class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-50 transition">
                <svg class="w-4 h-4 text-green-500 fill-current" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                WhatsApp
              </a>
            </div>
          </div>
    ...
    {{-- Meta strip: glass pill style matching Copy/Share buttons --}}
    <div class="absolute top-8 sm:top-10 left-8 sm:left-[4rem] lg:left-[7rem] z-20 pointer-events-none max-w-[calc(100%-4rem)] sm:max-w-none">
      <div class="inline-flex flex-wrap items-center gap-x-3 gap-y-1 px-3 py-2 rounded-xl bg-slate-900/60 backdrop-blur-md border border-white/20 shadow-sm">
        @if($avgRating)
          <span class="flex items-center gap-1 font-bold text-white text-xs">
            @for($i=1; $i<=5; $i++)
              <svg class="w-3.5 h-3.5 {{ $i <= round($avgRating) ? 'text-accent-400' : 'text-white/30' }} fill-current" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
            @endfor
            <span class="ms-0.5">{{ number_format($avgRating, 1) }}</span>
            <span class="font-normal text-white/60">({{ trans_choice('website/provider_profile.reviews.review_count', $reviewCount, ['count' => $reviewCount]) }})</span>
          </span>
        @else
          <span class="text-white/80 text-xs font-semibold">{{ __('website/provider_profile.sections.no_reviews_yet') }}</span>
        @endif
        @if($firstServiceArea)
          <span class="flex items-center gap-1 text-white/80 text-xs font-medium">
            <svg class="w-3.5 h-3.5 shrink-0 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg>
            {{ $firstServiceArea->area?->name ?? $firstServiceArea->district?->name ?? $firstServiceArea->division?->name ?? 'Bangladesh' }}
            @if($firstServiceArea->district)
              <span class="text-white/50">· {{ $firstServiceArea->district->name }}</span>
            @endif
          </span>
        @endif
        @if($profile->years_of_experience)
          <span class="flex items-center gap-1 text-white/80 text-xs font-medium">
            <svg class="w-3.5 h-3.5 shrink-0 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            {{ __('website/provider_profile.sections.years_experience', ['years' => $profile->years_of_experience]) }}
          </span>
        @endif
        @if($profile->experience_level)
          <span class="capitalize text-xs text-white font-bold px-2 py-0.5 rounded-full bg-white/20 border border-white/30">{{ $profile->experience_level }}</span>
        @endif
      </div>
    </div>


    {{-- Content starts at the 50% midpoint of the cover image.
         Formula: (viewport - 2*side-padding) * (890/1600) / 2 + top-padding
         px-6 = 3rem total, pt-4 = 1rem top, so:
         calc((100vw - 3rem) * 890 / 1600 / 2 + 1rem) --}}
    <div class="relative z-20 pointer-events-none" style="padding-top: calc((100vw - 3rem) * 890 / 1600 / 2 + 1rem)">
      <div class="max-w-6xl mx-auto px-4 lg:px-6 pointer-events-auto">

        {{-- Logo + Name row --}}
        <div class="flex items-end gap-5 pb-5">
          {{-- Logo --}}
          <div class="shrink-0 relative z-20">
            @if($profile->logo)
              <img src="{{ asset('storage/'.$profile->logo) }}" class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl border-4 border-white shadow-xl object-cover bg-white">
            @else
              <div class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl border-4 border-white shadow-xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-3xl font-bold">
            ...
            @endphp
            <img src="{{ asset($badgeImage) }}" class="absolute -top-1.5 -end-1.5 w-8 h-8 sm:w-9 sm:h-9 object-contain rounded-full bg-white border border-slate-100 p-0.5 shadow-md" alt="{{ $role }}" title="{{ ucfirst($role) }}">
          </div>
          {{-- Name + badges --}}
          <div class="pb-1 flex-1 min-w-0">
            {{-- Language pills above the name --}}
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <button 
                x-data="{ 
                  ...
                  toggleSave() {
                    if (!{{ auth()->check() ? 'true' : 'false' }}) {
                      if (typeof PNotify !== 'undefined') {
                        PNotify.error({
                          ...
                          text: 'You need to login to save providers.',
                          delay: 3000,
                          styling: 'brighttheme'
                        });
                      }
                    ...
                    .then(data => {
                      ...
                      if (typeof PNotify !== 'undefined') {
                        PNotify.success({
                          title: this.saved ? 'Saved' : 'Removed',
                          text: data.message,
                          delay: 2000,
                          styling: 'brighttheme'
                        });
                    ...
                    .catch(error => {
                      if (typeof PNotify !== 'undefined') {
                        PNotify.error({
                          title: '{{ __('website/provider_card.error') }}',
                          text: error.message || '{{ __('website/provider_profile.save.error_unable') }}',
                          delay: 3000,
                          styling: 'brighttheme'
                        });
                      }
                ...
                class="ms-1 w-8 h-8 flex items-center justify-center rounded-full bg-white hover:bg-slate-50 border border-slate-200/80 shadow-xs transition-all duration-200 hover:scale-110 cursor-pointer pointer-events-auto"
                :class="loading ? 'opacity-50 pointer-events-none' : ''"
                aria-label="Save provider"
              >
                <i 
                  class="transition-colors duration-200 text-sm" 
                  :class="saved ? 'fa-solid fa-heart text-red-500' : 'fa-regular fa-heart text-slate-400 hover:text-red-500'"
                ></i>
              </button>
            </div>
            @if($profile->tagline)
              <p class="mt-1 text-sm text-slate-500 font-medium truncate">{{ $profile->tagline }}</p>
            @endif
          </div>
        </div>
  ...
  <div class="max-w-6xl mx-auto px-4 lg:px-6 pb-20 relative z-20">
    <div class="lg:grid lg:grid-cols-[1fr_320px] lg:gap-8 mt-4">
      ...
      <div class="min-w-0">
        ...
        {{-- Profile header --}}
        <div class="pt-1 pb-5 border-b border-slate-200">


          {{-- Mobile Contact Trigger & Action Buttons --}}
          <div class="mt-5 space-y-3 lg:hidden">
            {{-- Contact Reveal Trigger --}}
            <div class="w-full">
              <template x-if="!showContact">
                <button @click="viewContactDetails()" 
                        class="w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-primary-600 hover:bg-primary-700 text-white text-sm font-black transition shadow-sm select-none">
                    <svg x-show="loadingContact" class="animate-spin h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <svg x-show="!loadingContact" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <div x-show="showContact" x-transition class="space-y-3">
                <div class="grid grid-cols-2 gap-2">
                  <template x-if="primaryPhone">
                    <a :href="'tel:' + primaryPhone" class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-primary-50 text-slate-700 text-xs font-bold transition shadow-2xs">
                      <svg class="w-4 h-4 text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                      {{ __('website/provider_profile.contact.call') }}
                    </a>
                  </template>
                  <template x-if="whatsappNumber">
                    <a :href="'https://wa.me/' + whatsappNumber.replace(/[^0-9]/g, '')" target="_blank" rel="noopener" class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-green-500 hover:bg-green-600 text-white text-xs font-bold transition shadow-2xs">
                ...
                {{-- Revealed Mobile Social & Website --}}
                <div x-show="website || facebookUrl || instagramUrl || youtubeUrl" class="pt-2 border-t border-slate-100 mt-2">
                  <p class="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">{{ __('website/provider_profile.contact.web_socials') }}</p>
                  <div class="flex flex-wrap gap-1.5">
                    <template x-if="website">
                      <a :href="website" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-200 text-[11px] font-semibold text-slate-700 hover:bg-slate-100 transition shadow-3xs">
                        <svg class="w-3.5 h-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>
                        <span>{{ __('website/provider_profile.contact.website') }}</span>
                      </a>
                    </template>
                    <template x-if="facebookUrl">
                      <a :href="facebookUrl" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 border border-blue-200 text-[11px] font-semibold text-blue-700 hover:bg-blue-100 transition shadow-3xs">
                        <svg class="w-3.5 h-3.5 text-blue-600 fill-current" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>
                        <span>Facebook</span>
                      </a>
                    </template>
                    <template x-if="instagramUrl">
                      <a :href="instagramUrl" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-pink-50 border border-pink-200 text-[11px] font-semibold text-pink-700 hover:bg-pink-100 transition shadow-3xs">
                    ...
                    <template x-if="youtubeUrl">
                      <a :href="youtubeUrl" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-50 border border-red-200 text-[11px] font-semibold text-red-700 hover:bg-red-100 transition shadow-3xs">
            ...
            {{-- Submit Request & Message Actions on Mobile --}}
            <div class="grid grid-cols-2 gap-2 mt-4">
              <button @click="openRequest()" class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-bold text-sm shadow-sm transition">
              ...
              <button @click="openMessage()" class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-bold text-sm shadow-sm transition">
